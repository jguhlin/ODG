...
; pulsar actor's has "log" so we exclude it here.
...
(def batchdb-server
  (reify Server
    ...
    (handle-call 
      ...
      (case (first message)
        :connect (let 
                   [[^String db_path memory] (rest message)]
      ...
      ;[]) ; Return something so it doesn't block forever
                         ...
                         (if (:filter-fn data)
                           (some (fn [node-id] ((:filter-fn data) 
                                                node-id 
                                                 (.getNodeProperties db node-id)
                                                 (map 
                                                   (fn [^org.neo4j.graphdb.Label x] (.name x)) 
                                                   (.getNodeLabels db node-id)))) 
                                 results)
                           (first results))))]))))))
       ...
       :query (let [[data] (rest message)
                    index-name (:index data)
                    index-manager (:index-manager @state)
                    ^org.neo4j.unsafe.batchinsert.BatchInserterIndex idx (get-node-index index-manager index-name)
                    ^org.neo4j.unsafe.batchinsert.BatchInserter db (:db @state)]
                       ...
                       [id
                             (if (:filter-fn data)
                               (first (filter (fn [node-id] ((:filter-fn data) 
                                                             node-id 
                                                              (.getNodeProperties db node-id)
                                                              (map (fn [^org.neo4j.graphdb.Label x] (.name x)) (.getNodeLabels db node-id)))) results))
                               (first results))])))))
