...
(defn wrap-unwrap-dbxrefs [[properties labels]]
 (if (:dbxref properties)
   (let [dbxrefs (clojure.string/split (:dbxref properties) #",")
         kvs (into {}
               (map
                (fn [[k v]]
                  [(keyword k) v])
                (map (fn [x] (clojure.string/split x #":")) dbxrefs)))]
     [(merge properties kvs) labels])
   [properties labels]))

(defn wrap-parsekv-pairs-in-note [[properties labels]]
  (if (and
        (:note properties)
        (re-find #"(\w+):(\w+)" (:note properties)))
    (let [note (:note properties)
          kvs
           (into {}
             (map
              (fn [[_ k v]]
...
(defn create-node
  ...
  (-> [gff-entry []] ; Start node entry with no labels
    util/wrap-urldecode
    util/wrap-preferred-capitalization
    util/wrap-add-label-from-type
    util/wrap-add-missing-id
    wrap-unwrap-dbxrefs
    wrap-parsekv-pairs-in-note
      ...
      (let [nodes-raw (map create-node (gff/parse-reader rdr))
            ...
            nodes (doall
                    (map
                     (fn [x]
                      (-> x
                        (util/wrap-add-property :species species) ; Lots of data but makes queries easier
                        (util/wrap-add-property :version version)
